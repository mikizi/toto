(function () {
  ...
  const els = {
    ...
    activeFinal: document.getElementById("activeFinal"),
    activeChampion: document.getElementById("activeChampion"),
  ...
  function renderTeamLines() {
    document.querySelectorAll("[data-team-line]").forEach((el) => {
      el.innerHTML = teamMarkup(el.dataset.teamLine);
    });
  }

  function scenarioCard(scenario) {
    const finalistText = `${scenario.finalists[0]} x ${scenario.finalists[1]}`;
    const leader = scenario.leader;
            ...
            .map(
              (row) => `
                <div class="scenario-top-row">
                  ...
                  <span class="scenario-player">${html(row.name)}<span class="scenario-mobile-points">${fmt(row.projected)}</span></span>
  ...
  function setActiveScenario(id, shouldPushUrl) {
    ...
    els.activeFinal.textContent = `${scenario.finalists[0]} x ${scenario.finalists[1]}`;
    els.activeChampion.textContent = `${scenario.champion} lifts the trophy`;
  ...
  async function init() {
    ...
    if (window.FINAL_SCENARIO_SNAPSHOT?.scenarios?.length) {
      ...
      els.versionStamp.textContent = shortVersion(window.FINAL_SCENARIO_SNAPSHOT.version);
      renderReadyState();
    } else {
